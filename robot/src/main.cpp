...
// Lokal hjælper til 90 graders præcisions-drejning
void turn_90_degrees(const std::string& direction) {
    ...
    usleep(600000); // Justeres efter fysisk test på gulvet
    motor_stop();
...
int main() {
    ...
    if (!load_config("config.json")) {
        std::cerr << "[Advarsel] Fortsætter muligvis med standard- eller manglende data." << std::endl;
    }
    ...
    while (true) {
        if (is_robot_running()) {
            ...
            // Beregn kørte distance og opdater vektor-position
            ...
            // Send telemetri via MQTT til server/database
            if (mqtt_client) {
                Vector2D pos = get_position();
                send_telemetry(mqtt_client, pos.x, pos.y, get_heading(), 
                               get_coverage_percentage(), get_obstacle_count(), obstacle);
            }

            // Reaktiv hændelseshåndtering (Forhindring eller grænse)
            if (obstacle != "") {
                motor_stop();
